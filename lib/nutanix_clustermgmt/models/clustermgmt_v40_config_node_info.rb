...
module NutanixClustermgmt
  ...
  class ClustermgmtV40ConfigNodeInfo < ApiModelBase
    # UUID of the host.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NutanixClustermgmt::ClustermgmtV40ConfigNodeInfo` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NutanixClustermgmt::ClustermgmtV40ConfigNodeInfo`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@hypervisor_hostname.nil? && @hypervisor_hostname.to_s.length > 64
        invalid_properties.push('invalid value for "hypervisor_hostname", the character length must be smaller than or equal to 64.')
      end
    ...
    def valid?
      ...
      return false if !@hypervisor_hostname.nil? && @hypervisor_hostname.to_s.length > 64
    ...
    def hypervisor_hostname=(hypervisor_hostname)
      ...
      if hypervisor_hostname.to_s.length > 64
        fail ArgumentError, 'invalid value for "hypervisor_hostname", the character length must be smaller than or equal to 64.'
      end
    ...
    def hash
      [node_uuid, block_id, node_position, hypervisor_type, is_robo_mixed_hypervisor, hypervisor_hostname, hypervisor_version, nos_version, is_light_compute, ipmi_ip, digital_certificate_map_list, cvm_ip, hypervisor_ip, model, current_network_interface].hash
    end
