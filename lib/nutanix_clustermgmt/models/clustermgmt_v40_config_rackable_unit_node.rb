...
module NutanixClustermgmt
  ...
  class ClustermgmtV40ConfigRackableUnitNode < ApiModelBase
    # UUID of the host.
    attr_accessor :uuid

    # Controller VM Id.
    attr_accessor :svm_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NutanixClustermgmt::ClustermgmtV40ConfigRackableUnitNode` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NutanixClustermgmt::ClustermgmtV40ConfigRackableUnitNode`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'svm_id')
        self.svm_id = attributes[:'svm_id']
      end
