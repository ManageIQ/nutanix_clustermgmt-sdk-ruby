...
module NutanixClustermgmt
  ...
  class ClustermgmtV40ConfigUplinkInfo
    attr_accessor :cvm_ip

    # Uplink details for a controller VM.
    attr_accessor :uplink_list
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NutanixClustermgmt::ClustermgmtV40ConfigUplinkInfo`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] uplink_list Value to be assigned
    def uplink_list=(uplink_list)
      if uplink_list.nil?
        fail ArgumentError, 'uplink_list cannot be nil'
      end

      if uplink_list.length > 1024
        fail ArgumentError, 'invalid value for "uplink_list", number of items must be less than or equal to 1024.'
      end

      if uplink_list.length < 0
        fail ArgumentError, 'invalid value for "uplink_list", number of items must be greater than or equal to 0.'
      end
