...
module NutanixClustermgmt
  ...
  class ClustermgmtV40ConfigRackableUnit
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NutanixClustermgmt::ClustermgmtV40ConfigRackableUnit`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def model_name=(model_name)
      ...
      if model_name.to_s.length > 64
        fail ArgumentError, 'invalid value for "model_name", the character length must be smaller than or equal to 64.'
      end
    ...
    def nodes=(nodes)
      ...
      if nodes.length > 1024
        fail ArgumentError, 'invalid value for "nodes", number of items must be less than or equal to 1024.'
      end

      if nodes.length < 0
        fail ArgumentError, 'invalid value for "nodes", number of items must be greater than or equal to 0.'
      end
