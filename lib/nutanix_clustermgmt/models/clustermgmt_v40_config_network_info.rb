...
module NutanixClustermgmt
  ...
  class ClustermgmtV40ConfigNetworkInfo
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NutanixClustermgmt::ClustermgmtV40ConfigNetworkInfo`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] hci Value to be assigned
    def hci=(hci)
      if hci.nil?
        fail ArgumentError, 'hci cannot be nil'
      end

      if hci.length > 1024
        fail ArgumentError, 'invalid value for "hci", number of items must be less than or equal to 1024.'
      end

      if hci.length < 0
        fail ArgumentError, 'invalid value for "hci", number of items must be greater than or equal to 0.'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] so Value to be assigned
    def so=(so)
      if so.nil?
        fail ArgumentError, 'so cannot be nil'
      end

      if so.length > 1024
        fail ArgumentError, 'invalid value for "so", number of items must be less than or equal to 1024.'
      end

      if so.length < 0
        fail ArgumentError, 'invalid value for "so", number of items must be greater than or equal to 0.'
      end
